...
- **OS**: macOS
- **ブラウザ**: ヒアスルーは macOS 上の Google Chrome または Apple Safari（最新版推奨）。会議マイクは Chrome の最新版（Safari では出力先を切り替えられません）
...
1. **ヘッドホンを接続して装着します。**
2. ブラウザで画面を開き、**「ヘッドホンへヒアスルー」** をクリックします。
...
Zoom や Google Meet のマイクに、Hush で処理した声を渡す手順です。仮想ドライバは同梱していません。Safari では出力先を切り替えられないので、Chrome を使ってください。会議の出力先に Multi-Output Device は選ばないでください。

1. [BlackHole](https://existential.audio/blackhole/) をインストールします。
2. Chrome を完全に終了してから、もう一度開きます。
3. Chrome で Hush を開き、**「会議のマイクとして開始」** を押します。
4. 仮想デバイスが複数あるときは、使うデバイスを選びます。
5. Zoom または Meet のマイク入力に、Hush が使っているその仮想デバイスを選びます。名前は稼働中の画面に表示されます。
6. Zoom または Meet のスピーカーにヘッドホンを選びます。Hush の再生先は仮想デバイスなので、会議の音は会議アプリ側で聞きます。
7. macOS の「システム設定 → サウンド → 入力」は、内蔵マイクやヘッドホンのマイクなど実マイクのままにします。ここを BlackHole にすると、Hush は自分の声ではなくループバックを拾います。
8. 計測が終わるまで静かにしてください。
...
- **他アプリ向けマイククリーナー**: 署名済みの仮想オーディオドライバは内蔵していません。会議ルートは、自分で入れた BlackHole などのデバイスへ処理後の音を送るだけです。
